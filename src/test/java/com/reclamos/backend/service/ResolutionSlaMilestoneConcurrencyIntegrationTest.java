...
class ResolutionSlaMilestoneConcurrencyIntegrationTest {
    ...
    @Autowired private TicketSlaMilestoneService service;
    ...
    @Test void firstResponseCandidateIsProcessedOnceAndDoesNotPublishBeforeRouting() throws Exception {
        TicketSla sla = createFirstResponseTicketWithSla();
        Ticket ticket = sla.getTicket();
        assertTrue(slas.findPendingMilestones(NOW).stream()
                .anyMatch(candidate -> candidate.slaId().equals(sla.getId())));
        ...
        assertEquals(0, database.queryForObject(
                "SELECT COUNT(*) FROM outbox_events WHERE ticket_id=?", Integer.class, ticket.getId()));
        assertEquals("FIRST_RESPONSE", database.queryForObject(
                "SELECT metadata->>'slaType' FROM ticket_activities WHERE ticket_id=? "
                        + "AND action_type='SLA_BREACHED'", String.class, ticket.getId()));
        cleanup(ticket.getId());
    }

    @Test void pausedResolutionIsNotReturnedAsSchedulerCandidate() {
        TicketSla sla = createTicketWithSla();
        sla.setPausedAt(NEAR.minusSeconds(60));
        slas.saveAndFlush(sla);

        assertFalse(slas.findPendingMilestones(NOW).stream()
                .anyMatch(candidate -> candidate.slaId().equals(sla.getId())));
        service.processCandidate(sla.getTicket().getId(), sla.getId(), NOW);
        assertEquals(SlaStatus.RUNNING, slas.findById(sla.getId()).orElseThrow().getStatus());
        assertEquals(0, count(sla.getTicket().getId(), ActivityType.SLA_NEAR_DUE));
        cleanup(sla.getTicket().getId());
    }
    ...
    private TicketSla createFirstResponseTicketWithSla() {
        RequestType requestType = requestTypes.findAll().stream().filter(RequestType::isActive).findFirst().orElseThrow();
        Ticket ticket = new Ticket();
        ticket.setPublicId("SLA-FIRST-" + UUID.randomUUID().toString().substring(0, 8));
        ...
        ticket.setSummary("Concurrencia SLA primera respuesta");
        ticket.setDescription("Ticket temporal para verificar FIRST_RESPONSE con FOR UPDATE");
        ...
        sla.setCycleNumber(1);
        sla.setPolicy(slaPolicies.findByPriorityAndSlaType(Priority.HIGH, SlaType.FIRST_RESPONSE).orElseThrow());
        sla.setStartedAt(NEAR.minusSeconds(3600));
        sla.setNearDueAt(NEAR);
        sla.setDueAt(DUE);
        sla.setStatus(SlaStatus.RUNNING);
