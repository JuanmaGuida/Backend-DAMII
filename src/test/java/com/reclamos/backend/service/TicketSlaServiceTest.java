...
class TicketSlaServiceTest {
    ...
    @Test void trackingDeadlineSnapshotUsesHistoricalFirstResponseAndLatestResolutionFromMainTicket() {
        Ticket main = new Ticket();
        main.setId(UUID.randomUUID());
        Ticket duplicate = new Ticket();
        duplicate.setId(UUID.randomUUID());
        duplicate.setMainTicket(main);
        TicketSla firstResponse = cycle(1, SlaStatus.MET);
        firstResponse.setTicket(main);
        firstResponse.setSlaType(SlaType.FIRST_RESPONSE);
        firstResponse.setDueAt(NEAR);
        TicketSla latestResolution = cycle(3, SlaStatus.RUNNING);
        latestResolution.setTicket(main);
        latestResolution.setDueAt(DUE);
        when(slas.findFirstByTicket_IdAndSlaTypeOrderByCycleNumberDesc(main.getId(), SlaType.FIRST_RESPONSE))
                .thenReturn(Optional.of(firstResponse));
        when(slas.findFirstByTicket_IdAndSlaTypeOrderByCycleNumberDesc(main.getId(), SlaType.RESOLUTION))
                .thenReturn(Optional.of(latestResolution));

        TicketSlaService.DeadlineSnapshot snapshot = service.findDeadlineSnapshot(duplicate);

        assertEquals(NEAR, snapshot.firstResponseDueAt());
        assertEquals(DUE, snapshot.resolutionDueAt());
    }
